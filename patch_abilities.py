...
def main():
    ...
    new_abilities = """window.abilities = {
    "Fire": [
    ...
    # We need to find `window.abilities = { ... };` and replace it
    # We will search for window.abilities = { and the matching closing };
    start_idx = html.find('window.abilities = {')
    if start_idx != -1:
        # find the next `};` after start_idx
        end_idx = html.find('};\n', start_idx) + 2

        # let's find the `const statsArr = ` which is right after abilities
        end_idx_fallback = html.find('const statsArr =', start_idx)
        if end_idx_fallback != -1:
            end_idx = html.rfind('};\n', start_idx, end_idx_fallback) + 2
            if end_idx > start_idx:
                html = html[:start_idx] + new_abilities + html[end_idx:]
                with open('index.html', 'w', encoding='utf-8') as f:
                    f.write(html)
                print("Successfully patched window.abilities")
            else:
                print("Could not find end of window.abilities")
        else:
            print("Could not find statsArr")
    else:
