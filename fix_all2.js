...
// The replacement was happening multiple times if the string existed multiple times or script ran multiple times
// I've fresh checked out. Let's do string replacement very specifically.
...
// DAY NIGHT CYCLE
// 1. Declare globals using strict replacement
html = html.replace(
    /let battleLogQueue = \[\];(\r?\n)let isTyping = false;/,
...
`;
html = html.replace(
    /popupBackground\.setVisible\(false\);\s*popupText\.setVisible\(false\);/,
...
`;
html = html.replace(
    /popupText\.setPosition\(gameSize\.width \/ 2, gameSize\.height - 100\);\s*}\);/,
...
`;
html = html.replace(
    /const frame = Math\.floor\(this\.time\.now \/ 150\) % 12;\s*bgGrass\.setTexture\('grass_' \+ frame\);/,
